...
interface ComparisonTableProps {
  ...
  const allFeatures = [
    ...
    { label: "Studies techniques", free: false, pro: true },
    ...
    { label: "Présentation par commande", free: true, pro: true },
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-default">
            <th className="text-left py-4 px-4 font-mono-caps text-secondary">
              Fonctionnalité
            </th>
            {plans.map((plan) => (
              <th
                key={plan.name}
                className="text-center py-4 px-4 font-medium text-primary"
              >
                {plan.name}
              </th>
            ))}
        ...
        <tbody>
          {allFeatures.map((feature, i) => (
            ...
            >
              <td className="py-3 px-4 text-secondary">{feature.label}</td>
              <td className="py-3 px-4 text-center">
                {typeof feature.free === "boolean" ? (
                  feature.free ? (
                    <Check className="w-4 h-4 text-accent mx-auto" />
                  ) : (
                    <X className="w-4 h-4 text-secondary mx-auto" />
                  )
                ) : (
