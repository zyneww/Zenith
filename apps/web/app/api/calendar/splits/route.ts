...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`https://finnhub.io/api/v1/stock/split?${params}`, {
      headers: { "X-Finnhub-Token": process.env.FINNHUB_API_KEY || "" },
    });
    if (!res.ok) throw new Error(`Finnhub split ${res.status}`);
    const json = await res.json();
    return NextResponse.json({ splits: json.data || json, source: "finnhub" });
  } catch {
